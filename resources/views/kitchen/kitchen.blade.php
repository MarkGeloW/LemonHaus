...
<div class="min-h-screen flex bg-[#f6f7fb]">
    ...
    <main class="flex-1 w-full px-4 sm:px-6 lg:px-10 pt-[96px] lg:pt-8 pb-10 overflow-x-hidden relative">
                        ...
                        @forelse($pendingOrders as $order)
                            <div class="rounded-[22px] border border-[#e9edf3] bg-white shadow-sm p-5 sm:p-7">
                                ...
                                <form id="accept-form-{{ $order->id }}"
                                      action="{{ route('kitchen.orders.accept', $order->id) }}"
                                      method="POST">
                                    @csrf
                                    @method('PATCH')
                        ...
                        @forelse($inProgressOrders as $order)
                            <div class="rounded-[22px] border border-[#e9edf3] bg-white shadow-sm p-5 sm:p-7">
                                ...
                                <form id="ready-form-{{ $order->id }}"
                                      action="{{ route('kitchen.orders.ready', $order->id) }}"
                                      method="POST">
                                    @csrf
                                    @method('PATCH')
    ...
    <div id="accept-modal" class="fixed inset-0 z-[120] hidden flex items-center justify-center bg-[#0f172a]/60 backdrop-blur-sm transition-opacity duration-300 opacity-0 px-4">
        <div id="accept-modal-content" class="bg-white rounded-[28px] sm:rounded-[32px] p-6 sm:p-8 max-w-md w-full shadow-2xl transform scale-95 transition-transform duration-300">
            <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-blue-50 flex items-center justify-center mx-auto mb-6 border-[8px] border-blue-100/50">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 sm:w-10 sm:h-10 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div id="ready-modal" class="fixed inset-0 z-[120] hidden flex items-center justify-center bg-[#0f172a]/60 backdrop-blur-sm transition-opacity duration-300 opacity-0 px-4">
        <div id="ready-modal-content" class="bg-white rounded-[28px] sm:rounded-[32px] p-6 sm:p-8 max-w-md w-full shadow-2xl transform scale-95 transition-transform duration-300">
            <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-green-50 flex items-center justify-center mx-auto mb-6 border-[8px] border-green-100/50">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 sm:w-10 sm:h-10 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
