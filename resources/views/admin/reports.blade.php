        ...
        <div class="max-w-[1430px] mx-auto">
            ...
            <div class="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-5 mb-8">
                <div>
                    ...
                    <p class="text-[15px] sm:text-[18px] text-[#64748b] mt-3">
                        Sales performance and inventory insights for the last {{ $days }} days
                    </p>
                </div>

                <div class="flex flex-col sm:flex-row sm:items-center gap-4 w-full xl:w-auto">

                    <form action="{{ route('reports.index') }}"
                          method="GET"
                          class="w-full sm:w-auto inline-flex items-center gap-2 rounded-2xl border border-[#e5e7eb] bg-white px-5 py-2 shadow-sm hover:bg-slate-50 transition">
                        <svg xmlns="http://www.w3.org/2000/svg"
                             class="w-5 h-5 text-[#64748b] shrink-0"
                             fill="none"
                             viewBox="0 0 24 24"
                             stroke="currentColor"
                             stroke-width="1.9">
                        ...
                        <select name="days"
                                ...
                                class="w-full bg-transparent border-none text-[15px] sm:text-[16px] font-semibold text-[#334155] focus:ring-0 cursor-pointer outline-none py-2">
                    ...
                    <a href="{{ route('reports.export', ['days' => $days]) }}"
                       class="w-full sm:w-auto inline-flex items-center justify-center gap-3 rounded-2xl bg-[#f4c400] px-6 sm:px-7 py-4 text-[16px] sm:text-[18px] font-semibold text-white shadow-md hover:bg-[#eab308] transition duration-200">
                        <svg xmlns="http://www.w3.org/2000/svg"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 mb-7">
                <div class="bg-white rounded-[22px] border border-[#e9edf3] shadow-sm p-5 sm:p-7">
                    ...
                    <h2 class="text-[24px] sm:text-[28px] font-extrabold text-[#0f172a] mt-3 break-words">
                ...
                <div class="bg-white rounded-[22px] border border-[#e9edf3] shadow-sm p-5 sm:p-7">
                    ...
                    <h2 class="text-[24px] sm:text-[28px] font-extrabold text-[#0f172a] mt-3">
                        {{ number_format($totalOrders) }}
                    </h2>
                </div>

                <div class="bg-white rounded-[22px] border border-[#e9edf3] shadow-sm p-5 sm:p-7 sm:col-span-2 xl:col-span-1">
                    <p class="text-[15px] sm:text-[16px] text-[#64748b] font-medium">
                        Avg. Order Value
                    </p>
            ...
            <div class="grid grid-cols-1 2xl:grid-cols-[1.9fr_1fr] gap-6 items-start">
                ...
                <div class="bg-white rounded-[24px] border border-[#e9edf3] shadow-sm p-5 sm:p-7 overflow-hidden">
                    ...
                    <div class="overflow-x-auto pb-4">
                        <div class="relative h-[340px] sm:h-[360px] min-w-[680px]">
                            @php
                                ...
                                $barWidthClass = $days > 14
                                    ? 'w-[14px] md:w-[20px] lg:w-[28px]'
                                    : 'w-[40px] md:w-[60px] lg:w-[88px]';

                                $fontSizeClass = $days > 14
                                    ? 'text-[10px] md:text-[12px]'
                                    : 'text-[14px] md:text-[16px]';
                            @endphp

                            <div class="absolute inset-0 flex flex-col justify-between text-[#94a3b8] text-[13px] pointer-events-none">
                                <div class="border-b border-dashed border-[#d1d5db] pb-1 pl-12">
                            ...
                            <div class="absolute inset-0 flex items-end justify-between px-12 pb-10 pt-10">
                                @forelse($weeklySalesData as $bar)
                                    <div class="flex flex-col items-center justify-end gap-3 group relative h-full flex-1">
                                        ...
                                        <span class="{{ $fontSizeClass }} text-[#64748b] font-medium absolute -bottom-8 whitespace-nowrap">
                                            {{ $bar['day'] }}
                                        </span>
                                    </div>
                                @empty
                ...
                <div class="bg-white rounded-[24px] border border-[#e9edf3] shadow-sm p-5 sm:p-7 min-h-[420px] flex flex-col">
                    <h2 class="text-[20px] sm:text-[22px] font-bold text-[#0f172a] mb-6">
                        Top Selling Items
                    </h2>

                    @if($topItems->isEmpty())
                        <div class="flex-1 flex items-center justify-center text-[#94a3b8] text-[16px] text-center">
                            No sales data available yet.
                        </div>
                    @else
                        <div class="flex flex-col items-center justify-center flex-1">
                            <div class="relative w-[190px] h-[190px] sm:w-[220px] sm:h-[220px] mb-8 sm:mb-10">
                                @php
                                ...
                                <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                    <span class="text-[24px] sm:text-[28px] font-extrabold text-[#0f172a]">
                                        {{ $totalTopQty }}
                                    </span>

                                    <span class="text-[12px] sm:text-[13px] text-[#64748b] font-medium -mt-1">
                                        Items Sold
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="w-full space-y-4">
                                @foreach($topItems as $index => $item)
                                    <div class="flex items-center justify-between gap-4 text-[14px] sm:text-[15px] font-medium">
                                        <div class="flex items-center gap-3 min-w-0">
                                            <div class="w-4 h-4 rounded-md shrink-0"
                                                 style="background-color: {{ $colors[$index % count($colors)] }}">
                                            </div>

                                            <span class="text-[#334155] truncate">
                                                {{ $item->product_name }}
                                            </span>
                                        </div>

                                        <span class="text-[#0f172a] font-bold whitespace-nowrap">
