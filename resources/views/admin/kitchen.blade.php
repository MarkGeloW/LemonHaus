...
<div class="min-h-screen flex bg-[#f6f7fb]">
                    ...
                    <div class="p-5 space-y-5 overflow-y-auto flex-1">
                        @forelse($pendingOrders as $order)
                            <div class="rounded-[22px] border border-[#e9edf3] bg-white shadow-sm p-7">
                                ...
                                <form id="accept-form-{{ $order->id }}" action="{{ route('admin.kitchen.orders.accept', $order->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                    ...
                    <div class="p-5 space-y-5 overflow-y-auto flex-1">
                        @forelse($inProgressOrders as $order)
                            <div class="rounded-[22px] border border-[#e9edf3] bg-white shadow-sm p-7">
                                ...
                                <form id="ready-form-{{ $order->id }}" action="{{ route('admin.kitchen.orders.ready', $order->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                    ...
                    <div class="p-5 space-y-5 overflow-y-auto flex-1">
                        @forelse($readyOrders as $order)
                            <div class="rounded-[22px] border border-[#e9edf3] bg-white shadow-sm p-7">
                                ...
                                <form id="archive-form-{{ $order->id }}" action="{{ route('admin.kitchen.orders.complete', $order->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
    ...
    {{-- ACCEPT MODAL --}}
    <div id="accept-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-[#0f172a]/60 backdrop-blur-sm opacity-0">
    ...
    {{-- READY MODAL --}}
    <div id="ready-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-[#0f172a]/60 backdrop-blur-sm opacity-0">
