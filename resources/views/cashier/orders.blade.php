...
<div class="min-h-screen flex bg-[#f6f7fb]">
            ...
            <div class="bg-white rounded-[24px] border border-[#e9edf3] p-7 mb-8 shadow-sm">
                ...
                <form id="create-order-form" action="{{ route('cashier.orders.store') }}" method="POST" class="space-y-6">
                    ...
                    <button type="button" id="add-product"
                        class="text-blue-500 font-medium hover:underline">+ Add Another Product</button>

                    <!-- Trigger Custom Modal instead of direct submit -->
                        ...
                        <tbody class="divide-y divide-[#eef2f7]">
                            @foreach($orders->groupBy('customer_name') as $customerName => $customerOrders)
                                <tr class="bg-slate-50/50">
                                    <td colspan="5" class="px-8 py-4 font-bold text-[#0f172a] border-b border-[#eef2f7] uppercase tracking-wide text-sm">
                                        {{ $customerName }}
                                    </td>
                                </tr>
                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td class="px-8 py-5 text-right">
                                            <form id="delete-form-{{ $order->id }}" action="{{ route('cashier.orders.destroy', $order->id) }}" method="POST">
                                                ...
                                                <button type="button" onclick="openDeleteModal('{{ $order->id }}', '{{ addslashes($order->customer_name) }}')" 
                            ...
                            @if($orders->isEmpty())
                                <tr>
                                    <td colspan="5" class="px-8 py-10 text-center text-[#64748b]">No active orders found.</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
    ...
    <div id="create-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-[#0f172a]/60 backdrop-blur-sm transition-opacity duration-300 opacity-0">
        <div id="create-modal-content" class="bg-white rounded-[32px] p-8 max-w-md w-full mx-4 shadow-2xl transform scale-95 transition-transform duration-300">
            ...
            <p class="text-[16px] text-[#64748b] text-center mb-8 px-4">
                Are you sure you want to send this order to the kitchen? Inventory will be updated automatically.
            </p>
            <div class="flex gap-4">
                <button type="button" onclick="closeCreateModal()" class="flex-1 py-4 rounded-2xl font-bold text-[#64748b] bg-slate-100 hover:bg-slate-200 transition text-[17px]">
                    Cancel
                </button>
                <button type="button" onclick="submitCreateForm()" class="flex-1 py-4 rounded-2xl font-bold text-slate-900 bg-[#f4c400] hover:bg-[#eab308] shadow-md transition text-[17px]">
                    Yes, Create
                </button>
            </div>
        </div>
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-[#0f172a]/70 backdrop-blur-sm transition-opacity duration-300 opacity-0">
        <div id="delete-modal-content" class="bg-white rounded-[32px] p-8 max-w-md w-full mx-4 shadow-2xl transform scale-95 transition-transform duration-300">
            ...
            <p class="text-[16px] text-[#64748b] text-center mb-8 px-4">
                Are you sure you want to permanently delete the order for <span id="delete-customer-name" class="font-bold text-slate-800"></span>? This cannot be undone.
            </p>
            <div class="flex gap-4">
...
<script>
    ...
    function openCreateModal() {
        // Force HTML5 validation before opening modal (ensures fields aren't empty)
