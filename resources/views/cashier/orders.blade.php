...
<div class="min-h-screen flex bg-[#f6f7fb]">
    ...
    <main class="flex-1 w-full px-4 sm:px-6 lg:px-10 pt-[96px] lg:pt-10 pb-10 overflow-x-hidden relative">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full min-w-[850px] text-left">
                        ...
                        <tbody class="divide-y divide-[#eef2f7]">
                            @if($orders->isEmpty())
                                <tr>
                                    <td colspan="5" class="px-8 py-10 text-center text-[#64748b]">
                            ...
                            @else
                                @foreach($orders->groupBy('customer_name') as $customerName => $customerOrders)
                                    <tr class="bg-slate-50/50">
                                        <td colspan="5" class="px-6 sm:px-8 py-4 font-bold text-[#0f172a] border-b border-[#eef2f7] uppercase tracking-wide text-sm">
                                            {{ $customerName }}
                                        </td>
                                    </tr>
                                        ...
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td class="px-6 sm:px-8 py-5 text-right whitespace-nowrap">
                                                <form id="delete-form-{{ $order->id }}"
                                                      action="{{ route('cashier.orders.destroy', $order->id) }}"
                                                      method="POST">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <div id="create-modal" class="fixed inset-0 z-[120] hidden flex items-center justify-center bg-[#0f172a]/60 backdrop-blur-sm transition-opacity duration-300 opacity-0 px-4">
        <div id="create-modal-content" class="bg-white rounded-[28px] sm:rounded-[32px] p-6 sm:p-8 max-w-md w-full shadow-2xl transform scale-95 transition-transform duration-300">
            ...
            <p class="text-[15px] sm:text-[16px] text-[#64748b] text-center mb-8 px-2 sm:px-4">
                Are you sure you want to send this order to the kitchen? Product stock will update automatically.
            </p>
